...
`autoHealAndApprove()` (`api/services/healService.js`) the instant a run comes back degraded, no
click needed to fix it. The dashboard's heal log still shows every step live (diagnosis,
...
`scripts/ci-monitor.js` alike, with no duplicated orchestration logic across those three trigger
sources.
...
```
This is runtime state (`collectors/ai-state.json`, via `lib/aiState.js`), not an env var. That's
deliberate: an env var is only read once at process startup, so editing `.env` while a
...
`needs_review`: the green checkmark itself is evidence the self-healing loop is working, not
...
required to run the app): `node scripts/screenshot.js http://localhost:3000 out.png`, which
