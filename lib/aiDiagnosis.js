...
Respond with ONLY the diagnosis text, under ${MAX_DIAGNOSIS_CHARS} characters, no preamble.`;
	...
	debugLog(
		"ai-diagnosis",
		`calling Gemini (${GEMINI_MODEL}) with ${reasons.length} reason(s), prompt ${prompt.length} chars`,
	);
	...
	const result = truncated
		? text.slice(0, MAX_DIAGNOSIS_CHARS - 3).replace(/\s+\S*$/, "") + "..."
		: text;
	debugLog(
		"ai-diagnosis",
		`Gemini responded with ${text.length} chars${truncated ? " (truncated to fit heal's limit)" : ""}`,
	);
	return result;
}
